...
- (id)initWithRecordable:(id<SRRecordable>)recordable options:(CRRecorderOptions)options {
  if ((self = [super init])) {
    ...
    if ((_options & CRRecorderOptionUserCameraRecording) == CRRecorderOptionUserCameraRecording) {
      ...
      _userRecorder = [[SRCameraRecorder alloc] init];
    ...
    for (id<SRRecordable> r in _recordables) {
      CGSize size = [r size];
...
- (BOOL)start:(NSError **)error {
  ...
  for (id<SRRecordable> recordable in _recordables) {
...
- (BOOL)_renderTouch:(SRUITouch *)touch context:(CGContextRef)context {
...
- (void)_renderTouchesInContext:(CGContextRef)context {
  ...
  for (SRUITouch *touch in _touches) {
...
//      if (touch.phase == UITouchPhaseEnded) {
        ...
        SRUITouch *crTouch = [[SRUITouch alloc] initWithPoint:p];
...
- (void)saveToAlbumWithName:(NSString *)name resultBlock:(CRRecorderSaveResultBlock)resultBlock failureBlock:(CRRecorderSaveFailureBlock)failureBlock {
  if (!_video) {
    if (failureBlock) failureBlock([SRUtils sr_errorWithDomain:CRErrorDomain code:CRErrorCodeInvalidVideo localizedDescription:@"No video available to save."]);
    return;
  }
